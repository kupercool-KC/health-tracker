...
| messages  | ChatMessage[]   | `{ role: "user"\|"assistant", content, createdAt, pendingMeal? }` |
...
write can't fabricate an assistant reply. A `pendingMeal` on an assistant
message means "proposed, not yet saved"; confirming it calls the existing
`POST /api/nutrition` with the already-parsed result attached (see below).
