...
interface DayInfo {
  ...
  netCalories: number;
  /** Raw total from the day's DailySteps doc. */
  steps: number;
  /** Estimated portion of `steps` generated by that day's logged workouts. */
  workoutSteps: number;
  /** steps − workoutSteps, floored at 0 — "everyday activity" steps. */
...
function MetricBarChart({
  ...
  days: DayInfo[];
  valueKey: "calories" | "protein" | "overallScore" | "steps" | "activitySteps";
  goal: number;
  label: string;
    ...
    (async () => {
      ...
      try {
        ...
        for (let i = 0; i < spanDays; i++) {
          ...
          const workoutSteps = estimateTotalWorkoutSteps(dayWorkouts);
          const activitySteps = Math.max(0, steps - workoutSteps);
    ...
    <main>
      ...
      {loading ? (
        ...
        <>
          ...
          <MetricBarChart
            days={days}
            valueKey={goals.stepsGoalExcludesWorkouts === false ? "steps" : "activitySteps"}
        ...
        >
          ...
          {selected.steps > 0 && (
            <p style={{ color: "var(--burned)", margin: "4px 0 0" }}>
              {t("steps")}:{" "}
              <bdi dir="ltr">
                {goals.stepsGoalExcludesWorkouts === false ? selected.steps : selected.activitySteps}
              </bdi>{" "}
              / <bdi dir="ltr">{goals.stepGoal ?? 8000}</bdi>
              {selected.workoutSteps > 0 && (
                <>
                  {" "}
                  ·{" "}
                  <bdi dir="ltr">
                    {selected.steps} = {selected.activitySteps} {t("stepsFromActivity")} + {selected.workoutSteps}{" "}
                    {t("stepsFromWorkouts")}
                  </bdi>
                </>
              )}
